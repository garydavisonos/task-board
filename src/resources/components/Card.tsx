...
const Card = ({label, description, deadline, id, listId, completed} : CardProps) => {
    ...
    return (
        <article className="bg-white rounded text-sm p-4 flex items-center flex-1">
            ...
            <ul className="ml-auto flex gap-2 text-xs">
                {!completed && <li><TextButton label="View" className="text-blue-500" onClick={() => openCardModal({label, description, deadline, id, listId, completed})} /></li> }
                <li><TextButton label="Delete" className="text-red-500" onClick={() => removeCard(listId, id)}  /></li>
            </ul>
        </article>
    )
}
